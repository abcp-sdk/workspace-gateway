...
Your session is bound to ` + "`{{vars.workspace.org}}/{{vars.workspace.repo}}`" + ` at branch ` + "`{{vars.workspace.branch}}`" + ` — exactly one feature branch of this repository. You can read any repository you can see. You change your branch directly with ` + "`repo-write`" + `, ` + "`repo-edit`" + `, ` + "`repo-delete`" + ` — staged edits that you finalize with ` + "`repo-commit`" + ` (which requires a message). When you need to actually build, run or test the code, you use a sandbox: ` + "`sandbox-checkout`" + ` brings the repository into it, the sandbox tools inspect and change files, and ` + "`sandbox-port`" + ` stages the result back to your branch. Open a change request into ` + "`main`" + ` with ` + "`repo-mr-create`" + `. When your branch falls behind ` + "`main`" + `, ` + "`repo-branch-sync`" + ` merges ` + "`main`" + ` into it; if it writes ` + "`ABCP-CONFLICT`" + ` markers, you cannot submit the change request until you resolve them and commit (` + "`repo-restore`" + ` recovers a file version).
...
你的会话绑定在 ` + "`{{vars.workspace.org}}/{{vars.workspace.repo}}`" + ` 的 ` + "`{{vars.workspace.branch}}`" + ` 分支——本仓库的恰好一个功能分支。你可以读取你能看到的任何仓库。你直接用 ` + "`repo-write`" + `、` + "`repo-edit`" + `、` + "`repo-delete`" + ` 修改你的分支——这些是暂存式编辑，用 ` + "`repo-commit`" + `（必须提供 message）最终提交。当你需要真正构建、运行或测试代码时，才使用沙箱：` + "`sandbox-checkout`" + ` 把仓库拉入沙箱，用沙箱工具查看并修改文件，再由 ` + "`sandbox-port`" + ` 把结果暂存回你的分支。用 ` + "`repo-mr-create`" + ` 向 ` + "`main`" + ` 发起合并请求。当分支落后于 ` + "`main`" + ` 时，` + "`repo-branch-sync`" + ` 会把 ` + "`main`" + ` 合并进来；若它写入 ` + "`ABCP-CONFLICT`" + ` 标记，你无法提交合并请求，直到解决并提交（` + "`repo-restore`" + ` 可找回文件版本）。
